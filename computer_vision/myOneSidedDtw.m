function [Dist,D,k,w,rw,tw]=myOneSidedDtw(r,t,pflag)
%
%  [Dist,D,k,w,rw,tw]=myOneSidedDtw(r,t,pflag)
%
% Dynamic Time Warping Algorithm restricted such that moves are only
% allowed for r but not for t
...
%       [values,number]=min([D(m-1,n),D(m,n-1),D(m-1,n-1)]);
      [values,number]=min([D(m-1,n),D(m-1,n-1)]);
      switch number
      case 1
        m=m-1;
%       case 2
